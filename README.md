...
7. Loading a mask with no `classes.json` traces it for you — every class in it becomes polygons with vertex handles you can reshape as in step 4, one record per connected region, holes kept as holes, and the status line reports what was traced. There is no trace button: a save that includes its `classes.json` is loaded exactly as written, so its polygons stay polygons and its brush paint stays paint. Traced polygons stay editable with the **Brush**: a brush or eraser stroke that touches one re-traces that neighbourhood from the pixels, so you can carve a hole with the eraser, fill one with the brush, split a region in two or bridge two into one — the stroke and its re-trace are a single **Undo** step. Traced pixels are replaced by the outlines, so mask and polygons always agree (edges may move by about 2 px), and a brush-edited polygon can differ from its pixels by about the same until the next stroke touches it. Paint you make here is never traced on its own — it stays paint, and a **Save Mask** / **Load Mask** round trip remembers which is which.
