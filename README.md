...
- **Trace Class — turn any mask into editable polygons** — a new **Trace Class** button in the Annotation panel traces the active class's pixels into polygon records with vertex handles, so a mask that arrived without polygons (a model's output, a colleague's segmentation, a legacy label, or your own brush paint) becomes reshapeable. Holes are kept as holes. The class's pixels are replaced by the traced outlines, so mask and polygons always agree; edges may move by about 1 px, and one **Undo** restores the original pixels and records. See [Annotation](#annotation) below.
- **Traced polygons are brush- and eraser-editable** — a brush or eraser stroke that touches a traced polygon re-traces that neighbourhood from the pixels, so carving a hole, filling one, splitting a region or bridging two all just work; the stroke and its re-trace are one **Undo** step. Hand-drawn polygons keep their eraser protection. See [Annotation](#annotation) below.
...
7. **Trace Class** traces the active class's pixels into polygons — one record per connected region, with holes kept as holes — so any loaded mask gets vertex handles you can reshape as in step 4. The class's pixels are replaced by the traced outlines (mask and polygons always agree; edges may move by about 1 px), and the status line reports what was traced (`Traced: 3 regions, 1 hole kept — pixels replaced`). Traced polygons stay editable with the **Brush**: a brush or eraser stroke that touches one re-traces that neighbourhood from the pixels, so you can carve a hole with the eraser, fill one with the brush, split a region in two or bridge two into one — the stroke and its re-trace are a single **Undo** step. Until you press **Trace Class** again, a brush-edited polygon can differ from its pixels by about 1 px. Traced polygons are saved and reloaded with the mask like any other, marked so their brush-editability survives a **Save Mask** / **Load Mask** round trip. A stroke that touches no traced polygon behaves exactly as before — hand paint is never traced on its own; the button is the only way to trace it.
8. **Undo / Redo** (Ctrl+Z / Ctrl+Y) operate at stroke granularity, depth 20 — one polygon fill, one brush stroke (with its re-trace, if any), one reshape, or one trace per step. **Clear Mask** wipes all classes' pixels (not undoable).
